    ...
    function imageview(){
      ...
      if (mysqli_num_rows($result) == 0) {
        ...
        while($row = mysqli_fetch_assoc($result)){
          ...
          $query2 = "SELECT * FROM pictures_tbl WHERE event_ID = '$event_id'";
          $result2 = mysqli_query($db,$query2) or die ("cannot complete add entry: ".mysqli_error($db));
          while($row2 = mysqli_fetch_assoc($result2)){
            $owner_ID = $row2['owner_ID'];
            $picID = $row2['picture_ID'];
            $shared = $row2['share'];
            $tagname = "";
            ...
            if($tagsear != "")
            {
              ...
              if($pos!== false)
              {
                if($filter == 1)
                {
                  ...
                  {
                    $count++;
                    $query3= "SELECT * FROM users WHERE user_ID = '$owner_ID'";
                    $result3 = mysqli_query($db,$query3) or die ("cannot complete add entry: ".mysqli_error($db));
                    $row3 = mysqli_fetch_assoc($result3);
                    $fname = $row3 ['f_name'];
                    $lname = $row3 ['l_name'];
                    $name = $row2['name'];
                    $imageFileNameAndLoc = $row2['filePath'];
                    $picID = $row2['picture_ID'];
                    $tagname = "";
                    $tags = structtags($picID,$tagname);;
                    $a = "nothing";
                    ...
                    echo '<a id = "ima" class="elem" href="../'.$imageFileNameAndLoc.'" tags = "'.$tags.'" title="'.$name.'" data-lcl-txt="nothing" data-lcl-author=" '.$fname.' '.$lname.'" >';
      ...
      {
        echo "There are $count results found with '$tagsear'.";
      }
    }
    imageview();
    ?>
