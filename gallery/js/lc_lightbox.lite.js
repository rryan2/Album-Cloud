...
(function ($) {
	...
	'<div id="lcl_wrap" class="lcl_pre_show lcl_pre_first_el lcl_first_sizing lcl_is_resizing">'+
		'<div id="lcl_window">'+
			...
			'</div>'+
			'<div id="tagsdiv" style = "opacity: 0.5; position:fixed; z-index: 50000; left: 100%; width:15%;	height:100%; background-color:black;">'+
			'</div>'+
			'<div id="dialog">'+
				'<p class="close"><a href="#" rel="external nofollow" >close</a></p>'+
				'<p><input id = "public" type="checkbox"><span style="color:white;font-weight:bold">Public</span></p>'+
			'<div id="dialist">'+
			'</div>'+
				'<div id="intag">'+
	...
	//	document.getElementById("dialist").appendChild(taginput);
	// initialization
	...
	var lcl = function(obj, settings) {
				...
				if(already_man) {
					...
					if(type != 'unknown') {
						...
						var el = {
							...
							tags : (o.show_tags) 	? attr_or_selector_data($e, 'tags_attr') : '',
							isowner : attr_or_selector_data($e, 'is_owner'),
							ispublic : attr_or_selector_data($e, 'is_public'),
							image: attr_or_selector_data($e, 'image_id'),
		...
		var size_elem = function(el, flags, txt_und_sizes) { // flags: no_txt_under, inner_cmd_checked
			...
			var list =	document.getElementById("tagsdiv");
			while(list.firstChild)
			{
				list.removeChild(list.firstChild);
			}
			var tagl = el.tags;
			var taglist = tagl.split(",");
			var taglist2 = tagl.split(",");
			taglist.forEach(function(e){
				...
				node.setAttribute("align", "center");// Create a text node
				node.appendChild(textnode);                              // Append the text to <li>
				document.getElementById("tagsdiv").appendChild(node);
			});
			...
			img.setAttribute('src', '../images/edit.png');            // Create a <li> node
			img.setAttribute("width", "30");
			img.setAttribute("height", "30");
			node.setAttribute("style", "padding-left:30%");// Create a text node
			...
			var dialist = document.getElementById("dialist");
			while(dialist.firstChild)
			{
				dialist.removeChild(dialist.firstChild);
			}
			taglist2.forEach(function(e){
				var node = document.createElement("h4");                 // Create a <li> node
				var textnode = document.createTextNode(e);
				node.setAttribute("style", "font-size: 20px");
				node.setAttribute("style", "color: white");
				node.setAttribute("align", "left");// Create a text node
				node.appendChild(textnode);                              // Append the text to <li>
				document.getElementById("dialist").appendChild(node);
			});
			//edit tag
			var imaid = el.image;
			imageid = imaid;
			var addtag = function(){
				...
				switch_elem("this");
				document.getElementById("addb").removeEventListener("click", addtag);
			};
			document.getElementById("addb").addEventListener("click", addtag);
			...
			var check = function()
			{
				if(imageid == el.image)
				{
					if(el.isowner == 1&&checkbox.checked == true)
					{
						$.ajax({
							url:"sever2.php",
							type:"post",
					...
					else if(el.isowner == 1&&checkbox.checked == false)
					{
						$.ajax({
							url:"sever2.php",
							type:"post",
			...
			document.getElementById("public").addEventListener("click", check);
